...
const handleRemoveBook = () => {
  fetch(`/user-books/${id}`, {method: 'DELETE'})
    .then(res => {
      if (res.ok) {
        removeUserBook(book)
      } else {
...
const bookInCollection = userBooks.some(book => book.title === title && book.author === author)
  return (
    <>
      ...
      <h3>{genre}</h3>
      <h3>{page_count}</h3>
      {!bookInCollection? <button onClick={handleAddBook}>Add to Library</button> : <button onClick={handleRemoveBook}> Remove From Library</button>}
    </>
    )
}
